...
//
// Modified to support JK01 printer (uses ae10 instead of ae01 for cat protocol)
//
...
BLEClientService myService;
BLEClientCharacteristic myDataChar;
...
char szTemp[32];
   ...
   ParseDeviceName(report->data.p_data, szTemp);
    ...
    if (ucPrinterType > PRINTER_COUNT) {
      ...
      bNRFFound = 1;
      ParseDeviceName(report->data.p_data, szPrinterName);
...
static BLEUUID CHAR_UUID_DATA_CAT_ALT(BLEUUID((uint16_t)0xae10));   // JK01 uses this
static BLEUUID CHAR_UUID_NOTIFY_CAT_ALT(BLEUUID((uint16_t)0xae3c)); // JK01 notify
...
class tpAdvertisedDeviceCallbacks: public BLEAdvertisedDeviceCallbacks
{
    void onResult(GeneralBLEAdvertisedDevice genAdvertisedDevice)
    {
      ...
      if (iLen > 0 && memcmp(advertisedDevice->getName().c_str(), szPrinterName, iLen) == 0)
      {
        ...
        if (ucType < PRINTER_COUNT) {
            Server_BLE_Address = new BLEAddress(advertisedDevice->getAddress());
            Scanned_BLE_Address = Server_BLE_Address->toString().c_str();
            ucPrinterType = ucType;
            strcpy(Scanned_BLE_Name, advertisedDevice->getName().c_str());
            strcpy(szPrinterName, Scanned_BLE_Name);
#ifdef DEBUG_OUTPUT
...
int miny, maxy;
   ...
   if (width == NULL || top == NULL || bottom == NULL || pFont == NULL || szMsg == NULL) return;
   miny = 100; maxy = 0;
   while (szMsg[i]) {
      c = szMsg[i++];
      if (c < pFont->first || c > pFont->last)
         continue;
      c -= pFont->first;
...
GFXglyph glyph, *pGlyph;
   ...
   while (szMsg[i] && x < bb_width)
   {
      ...
      dx = x + pGlyph->xOffset;
      dy = y + pGlyph->yOffset;
      s = pFont->bitmap + pGlyph->bitmapOffset;
      iBitOff = 0;
      bits = uc = 0;
...
int iPrintWidth = iPrinterWidth[ucPrinterType];
   ...
   for (y=miny; y<=maxy; y++)
   {
    ...
    tpSendScanline(ucTemp, (iPrintWidth+7)/8);
  }
  tpPostGraphics();
  return 0;
}
...
uint8_t bFlipped = false;
  ...
  if (cy > 0)
     bFlipped = true;
  if (cx + iXOffset > bb_width || cy + iYOffset > bb_height)
     return -1;
...
int tpConnect(const char *szMacAddress)
{
    ...
    if (pRemoteService != NULL)
    {
      ...
      pRemoteCharacteristicNotify = NULL;
      if (ucPrinterType == PRINTER_MTP2 || ucPrinterType == PRINTER_MTP3)
          pRemoteCharacteristicData = pRemoteService->getCharacteristic(CHAR_UUID_DATA0);
      else if (ucPrinterType == PRINTER_CAT)
      {
          // JK01 and similar use ae10/ae3c instead of ae01/ae02
          pRemoteCharacteristicData = pRemoteService->getCharacteristic(CHAR_UUID_DATA_CAT_ALT);
          pRemoteCharacteristicNotify = pRemoteService->getCharacteristic(CHAR_UUID_NOTIFY_CAT_ALT);
          if (pRemoteCharacteristicData == nullptr) // fallback to original for other cat printers
          {
      ...
      {
          if (pRemoteCharacteristicNotify != NULL && pRemoteCharacteristicNotify->canNotify())
  ...
  return 0;
    ...
    if (peripheral.connect())
    {
        ...
        prtService = peripheral.service(szServiceNames[ucPrinterType]);
        if (prtService)
        {
    ...
    long ulTime = millis();
    while (!bConnected && (millis() - ulTime) < 4000)
    {
        delay(20);
    }
    return bConnected;
...
int i = 0;
   ...
   while (szPrinterIDs[i].szBLEName != NULL) {
     if (strcmp(szName, szPrinterIDs[i].szBLEName) == 0) {
...
int iLen = strlen(szName);
    ...
    if (pBLEScan != NULL)
    {
      pBLEScan->setAdvertisedDeviceCallbacks(new tpAdvertisedDeviceCallbacks());
      pBLEScan->setActiveScan(true);
      bConnected = false;
      Server_BLE_Address = NULL;
      pBLEScan->start(iSeconds);
    }
    ulTime = millis();
    while (!bFound && (millis() - ulTime) < iSeconds*1000L)
    {
       if (iLen == 0 && ucPrinterType < PRINTER_COUNT) {
          pBLEScan->stop();
          bFound = 1;
       } else if (iLen > 0 && memcmp(Scanned_BLE_Name,szPrinterName, iLen) == 0) {
           pBLEScan->stop();
           bFound = 1;
    ...
    while (!bFound && (millis() - ulTime) < (unsigned)iSeconds*1000UL)
    {
        ...
        if (peripheral)
        {
            ...
            } else if (iLen == 0 && strlen(peripheral.localName().c_str()) > 0) {
              ucPrinterType = tpFindPrinterName((char *)peripheral.localName().c_str());
              if (ucPrinterType < PRINTER_COUNT) {
                   BLE.stopScan();
                   bFound = 1; 
  ...
  if (ucPrinterType == PRINTER_MTP2 || ucPrinterType == PRINTER_MTP3) {
    ...
    myDataChar = BLEClientCharacteristic(0xae10);  // JK01 uses ae10
    if (!myDataChar.discover()) {                  // fallback if needed
...
uint8_t ucTemp[10] = {0x51, 0x78, 0xCC, 0x00, 0x02, 0x00, 0xDD, 0xDD, 0xC0, 0xFF};
    ucTemp[2] = command;
    ucTemp[6] = (uint8_t)(data & 0xFF);
    ucTemp[7] = (uint8_t)(data >> 8);
...
int i;
  ...
  if (ucPrinterType == PRINTER_FOMEMO || ucPrinterType == PRINTER_MTP2 || ucPrinterType == PRINTER_MTP3 || ucPrinterType == PRINTER_PERIPAGE || ucPrinterType == PRINTER_PERIPAGEPLUS) {
     ...
     if (iUnderline) ucTemp[i] |= 0x80;
     if (iDoubleWide) ucTemp[i] |= 0x20;
     if (iDoubleTall) ucTemp[i] |= 0x10;
     if (iEmphasized) ucTemp[i] |= 0x8;
...
uint8_t storeQR[] = {0x1d, 0x28, 0x6b, 0x00, 0x00, 0x31, 0x50, 0x30};
...
int i=0;
   ...
   ucTemp[i++] = 0x1d; ucTemp[i++] = 0x77; ucTemp[i++] = 2;
  ...
  if (ucPrinterType == PRINTER_CAT) {
     ...
     for (int i = 0; i<iLen; i++)
     {
         ...
         if (pString[i] >= ' ') 
         {
             ...
             if (CatStrLen==48) tpPrintCatTextLine();
         }
     }
     return 1;
  }
...
uint8_t ucTemp[16];
  ...
  if (ucPrinterType == PRINTER_CAT) {
   ...
   for (i=0; i<iLines; i++) {
     ...
     ucTemp[4] = 1; ucTemp[5] = 0;
     ucTemp[6] = 1; ucTemp[7] = 0;
     ucTemp[8] = 0;
     tpWriteData(ucTemp, 9);
     delay(5);
   }
...
uint8_t ucTemp[16];
  ...
  if (ucPrinterType == PRINTER_CAT) {
    ...
    tpWriteData(ucTemp, 4);
    memset(ucTemp, 0, 12);
    tpWriteData(ucTemp, 12);
    ucTemp[0] = 0x1d; ucTemp[1] = 0x76;
    ucTemp[2] = 0x30; ucTemp[3] = 0x00;
    ucTemp[4] = (uint8_t)((iWidth+7)>>3); ucTemp[5] = 0x00;
    ucTemp[6] = (uint8_t)iHeight; ucTemp[7] = (uint8_t)(iHeight>>8);
    tpWriteData(ucTemp, 8);
  }
...
void tpDrawLine(int x1, int y1, int x2, int y2, uint8_t ucColor)
{
  ...
  if(abs(dx) > abs(dy)) {
    ...
    p = &pBackBuffer[(y1 * bb_pitch) + (x1 >> 3)];
    mask = 0x80 >> (x1 & 7);
    for(; x1 <= x2; x1++) {
      if (ucColor)
        *p |= mask;
