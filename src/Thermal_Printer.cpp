...
// 0xA5 ???
const uint8_t controlLattice = 0xA6;		// 0xA6 control Lattice Eleven bytes, all constants. One set used before printing, one after.
    ...
    if (pRemoteService != NULL)
    {
      ...
      if (pClient->isConnected())
      {
        ...
        if (ucPrinterType == PRINTER_MTP2 || ucPrinterType == PRINTER_MTP3)
          ...
          {
            pRemoteCharacteristicData = pRemoteService->getCharacteristic(CHAR_UUID_DATA_JK01);
            pRemoteCharacteristicNotify = pRemoteService->getCharacteristic(CHAR_UUID_NOTIFY_JK01);
...
int i = 0;
   ...
   while (szPrinterIDs[i].szName != NULL) {
     if (strcmp(szName, szPrinterIDs[i].szName) == 0) { // found a supported printer
#ifdef DEBUG_OUTPUT
     Serial.print("Found a match for ");
     Serial.println(szName);
     Serial.print("Printer type = ");
     Serial.println(szPrinterIDs[i].ucType, DEC);
#endif
        return szPrinterIDs[i].ucType;
     } else {
       i++;
     }
...
//    Bluefruit.Scanner.filterUuid(myService.uuid);
  ...
  if (ucPrinterType == PRINTER_MTP2 || ucPrinterType == PRINTER_MTP3) {
    ...
    myDataChar = BLEClientCharacteristic(0xae10); // JK01
    if (!myDataChar.discover()) { // fallback
